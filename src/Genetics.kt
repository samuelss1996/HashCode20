...
class Genetics(problem: Problem, val values: Array<Int>, val size: Int, val target: Int) : Algorithm(problem) {
    ...
    private fun generateIndividual(generationStrategy: GenerationStrategy): Individual {
        val solution = Array(size) {false}

        when(generationStrategy) {
            GenerationStrategy.GREEDY -> {
                // TODO generate greedy solution
            }
        ...
        return Individual(solution, this.cities.getCost(path))
    }
